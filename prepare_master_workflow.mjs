...
// Filter out old helper nodes
raw.nodes = raw.nodes.filter(n => n.name !== 'Tiene Correo Dueño?' && n.name !== 'Notificar Dueño - Nuevo Turno' && n.name !== 'Resolver Correo Dueño');
...
if (body.branding && body.branding.ownerEmail) {
  email = body.branding.ownerEmail.trim();
}
...
if (!email || !email.includes('@')) {
  try {
    const res = await fetch('https://estetica.neco-dev.cloud/branding.json');
    if (res.ok) {
      const data = await res.json();
      if (data.ownerEmail) email = data.ownerEmail.trim();
    }
  } catch (err) {}
}

return [{ json: { ownerEmail: email, hasOwnerEmail: email.includes('@') } }];
`
  },
  name: "Resolver Correo Dueño",
  type: "n8n-nodes-base.code",
  typeVersion: 2,
  position: [ 480, 240 ],
  id: "e1f2a3b4-5c6d-7e8f-9012-3456789abcde"
};
...
const nodeNotify = {
  parameters: {
    sendTo: "={{ $('Resolver Correo Dueño').first().json.ownerEmail }}",
...
raw.nodes.push(nodeResolver);
raw.nodes.push(nodeIf);
raw.nodes.push(nodeNotify);

raw.connections["Añadir Turno"] = {
  main: [
    [
      { node: "Enviar Correo Confirmación", type: "main", index: 0 },
      { node: "Resolver Correo Dueño", type: "main", index: 0 }
    ]
  ]
};
...
raw.connections["Resolver Correo Dueño"] = {
  main: [
    [
...
writeFileSync('./master_workflow.json', JSON.stringify(raw, null, 2), 'utf8');
console.log('master_workflow.json written successfully!');
